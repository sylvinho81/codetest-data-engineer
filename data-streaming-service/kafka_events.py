...
def gen_specific_message(file):
    content = read_file(f'{DATA_FOLDER}/{file}')
    msg = {
        "id": content["id"],
        "time": int(time.time()),
        "readers": random.randint(1e2, 1e5),
...
# while True:
for file in files:
    # I have just modified this, because if not it is picking ramdonly files from texts folder.
    # msg = gen_message(files)
    msg = gen_specific_message(file)
    p.poll(0)
